...
NAME
      B-Mercator: Inference of high-quality embeddings of bipartite complex networks into the
                   hyperbolic spaces

SYNOPSIS
      bmercator [options] <edgelist_filename>
...
The following options are available:
    ...
    -t             Track the global loglikelihood during the maximization likelihood phase
...
void parse_options(int argc , char *argv[], embeddingSD_t &the_graph)
{
  ...
  // Parsing options.
  int opt;
  while ((opt = getopt(argc,argv,"ab:cfkpo:r:qs:vd:em:tx")) != -1)
  {
    switch(opt)
    {
      case 'a':
      ...
      case 'm':
        the_graph.WITH_FEATURES = true;
        the_graph.FEATURES_FILENAME = optarg;
        break;

      case 't':
        the_graph.TRACK_GLOBAL_LOGLIKELIHOOD = true;
        break;

      case 'x':
        the_graph.BIPARTITE_MODE = true;
        break;
  ...
  // <edgelist_filename>
  if (the_graph.BIPARTITE_MODE)
    the_graph.EDGELIST_FILENAME = the_graph.FEATURES_FILENAME; 
  else
