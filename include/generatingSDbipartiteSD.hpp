#ifndef GENERATINGSDBIPARTITESD_HPP_INCLUDED
#define GENERATINGSDBIPARTITESD_HPP_INCLUDED

/**
 * Compile: g++ -O3 -std=c++17 -lboost_system -lboost_math_c99 src/generatingSDBipartiteSD_unix.cpp -o genSDbipartite
 * 
 */
...
class generatingSDBipartiteSD_t
{
  ...
  public:
    ...
    double BETA = -1; // Parameter beta (controls clustering).
    double MU = -1; // Parameter mu (controls average degree).
    double GAMMA = -1; // Exponent of the power-law distribution for hidden degrees.
    int NB_VERTICES = -1; // Network size
    double MEAN_DEGREE = -1; // Mean degree of nodes
  ...
  private:
    ...
    // Unipartite variables
    std::vector<double> kappa; // Hidden variables of the vertices.
    std::vector<double> theta; // Positions of the vertices.
    std::vector<std::vector<double>> d_positions; // Position of the vertices in D-dimensions
...
void generatingSDBipartiteSD_t::validate_input_parameters() 
{
  if (BETA <= DIMENSION) {
    throw std::invalid_argument("Parameter beta should be larger than D.");
  }
...
void generatingSDBipartiteSD_t::generate_powerlaw_distribution(int n, double gamma, double mean_degree, 
...
void generatingSDBipartiteSD_t::generate_bipartite_edgelist(int width)
{
  ...
  if (DIMENSION == 1) {
    ...
    // Correlation between nodes in unipartite and bipartite networks (approach 2)
    for (int v=0; v<NB_VERTICES; ++v) {
      double random_angle = 2 * POS_CORRELATION * uniform_01(engine) - POS_CORRELATION; // U(-x, x)
      double new_theta = theta[v] + random_angle;
      if (new_theta < 0) new_theta += 2 * PI;
      if (new_theta > 2 * PI) new_theta -= 2 * PI;
      theta_nodes[v] = new_theta;
    }
    ...
    // Correlation between nodes in unipartite and bipartite networks (approach 2)
    std::vector<double> origin(DIMENSION+1, 0.0);
    origin[0] = 1.0;
    for(int v=0; v<NB_VERTICES; ++v) {
      double random_angle = POS_CORRELATION * uniform_01(engine);
      std::vector<double> new_position = generate_random_d_vector_with_first_coordinate(DIMENSION, random_angle, radius);
      normalize_and_rescale_vector(new_position, 1.0);
      std::vector<double> tmp_position = d_positions[v];
      normalize_and_rescale_vector(tmp_position, 1.0);
      Eigen::MatrixXd mat = compute_rotation_matrix(origin, tmp_position);
      std::vector<double> rotated_new_position = rotate_vector(mat, new_position);
      normalize_and_rescale_vector(rotated_new_position, radius);
      d_positions_nodes[v] = rotated_new_position;
    }    
  ...
  for(int v=0; v<NB_VERTICES; ++v) {
    for(int f=0; f<NB_FEATURES; ++f) {
      ...
      const auto inside = (radius * dtheta) / std::pow(BIPARTITE_MU * kappa_nodes[v] * kappa_features[f], 1.0 / DIMENSION);
      const auto prob = 1 / (1 + std::pow(inside, BIPARTITE_BETA)); 
      
      if(uniform_01(engine) < prob)
      {
        edgelist_file << std::setw(width) << Num2Name[v] << " ";
...
void generatingSDBipartiteSD_t::generate_edgelist(int width)
{
  const double radius = compute_radius(DIMENSION, NB_VERTICES);
  ...
  if (BETA > DIMENSION) {
    ...
    while (true) {
      auto f = [&](double kappa1, double kappa2, double theta) {
        ...
        const double bottom = 1 + pow(radius * theta / pow(MU * kappa1 * kappa2, 1.0 / DIMENSION), BETA);
        return top / bottom;
      };
      double computedMeanDegree = prefactor * integrate_3d(f, kappa_0, kappa_c, kappa_0, kappa_c, 0, PI);
      
      if (fabs(computedMeanDegree - MEAN_DEGREE) < 0.1)
        break;
  ...
  if (DIMENSION == 1) {
    ...
    for(int v(0); v<NB_VERTICES; ++v)
      d_positions[v] = generate_random_d_vector(DIMENSION, radius);
  }
...
void generatingSDBipartiteSD_t::load_hidden_variables()
{
  ...
  if (WITH_INPUT_KAPPAS) {  
    ...
    generate_powerlaw_distribution(NB_VERTICES, GAMMA, MEAN_DEGREE, kappa, Num2Name); 
  }
}

void generatingSDBipartiteSD_t::save_bipartite_properties(std::vector<int>& rdegree_nodes, std::vector<int>&rdegree_features, 
                                                          std::vector<double>& edegree_nodes, std::vector<double>& edegree_features, int width)
{
  ...
  for(int v(0); v<NB_VERTICES; ++v)
  {
    hidden_variables_nodes_file << std::setw(width) << Num2Name[v]                                                                 << " ";
...
void generatingSDBipartiteSD_t::save_vertices_properties(std::vector<int>& rdegree, std::vector<double>& edegree, int width)
{
  // Finds the minimal value of kappa.
  double kappa_min = *std::min_element(kappa.begin(), kappa.end());
  const auto R = compute_radius(DIMENSION, NB_VERTICES);
  const double hyp_radius = 2 * std::log(2 * R / std::pow(MU * kappa_min * kappa_min, 1.0 / DIMENSION));
...
std::string generatingSDBipartiteSD_t::get_time()
{
...
std::vector<double> generatingSDBipartiteSD_t::generate_random_d_vector(int dim, double radius) {
...
double generatingSDBipartiteSD_t::compute_angle_d_vectors(const std::vector<double> &v1, const std::vector<double> &v2) {
...
inline double generatingSDBipartiteSD_t::compute_radius(int dim, int N) const
{
  const auto inside = N / (2 * std::pow(PI, (dim + 1) / 2.0)) * std::tgamma((dim + 1) / 2.0);
  return std::pow(inside, 1.0 / dim);
}

void generatingSDBipartiteSD_t::normalize_and_rescale_vector(std::vector<double> &v, double radius) {
  int dim = v.size() - 1;
  double norm=0;
...
Eigen::MatrixXd generatingSDBipartiteSD_t::compute_rotation_matrix(std::vector<double> &axis, std::vector<double> &v) {
...
std::vector<double> generatingSDBipartiteSD_t::rotate_vector(Eigen::MatrixXd &rotation_matrix, std::vector<double> &v) {
...
std::vector<double> generatingSDBipartiteSD_t::generate_random_d_vector_with_first_coordinate(int dim, double angle, double radius) {
...
#endif // GENERATINGSDBIPARTITESD_HPP_INCLUDED