...
double compute_bipartite_clustering(const std::vector<std::set<int>> &bipartite_adjacency_list1, 
  ...
  for (int i=0; i<size; ++i) // type 1 nodes
  {
    ...
    for (const int n1: neighbours_i)
    {
      for (const int n2: neighbours_i) 
      {
        ...
        num_triangles += intersect.size() > 1 ? 1 : 0; // do not include node i here
      }
    }
    bipartite_clustering += (2 * num_triangles) / ((neighbours_i_size) * (neighbours_i_size - 1));
  }
  return bipartite_clustering / size_degree_gr2 / 2; 
}
...
int main(int argc , char *argv[]) {
  ...
  const double cb_nodes = compute_bipartite_clustering(bipartite_adjacency_list_nodes, bipartite_adjacency_list_features);
  const double cb_features = compute_bipartite_clustering(bipartite_adjacency_list_features, bipartite_adjacency_list_nodes);
